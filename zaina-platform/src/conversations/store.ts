...
export async function getSession(sessionId: string): Promise<ChatSession | undefined> {
  ...
  const [row] = await platformDb().select().from(chatSessions).where(eq(chatSessions.id, sessionId)).limit(1);
  return row;
}
...
/** Successful results of one tool in this conversation, newest first. */

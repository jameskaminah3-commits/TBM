...
//      (PLATFORM_APP_DATABASE_URL), give that role the password in the URL,
//      so the database and the service always agree;
...
import { closePlatformDb, initPlatformDb, ownerPool, APP_ROLE } from "../db/platform-db.ts";
...
/** The app role's password from its connection URL, checked. */
export function appRolePassword(appUrl: string): string {
  let parsed: URL;
  try {
    parsed = new URL(appUrl);
  } catch {
    throw new Error("PLATFORM_APP_DATABASE_URL isn't a valid postgres:// address");
  }
  const user = decodeURIComponent(parsed.username);
  const password = decodeURIComponent(parsed.password);
  if (user !== APP_ROLE) throw new Error(`PLATFORM_APP_DATABASE_URL must sign in as ${APP_ROLE}, not "${user}"`);

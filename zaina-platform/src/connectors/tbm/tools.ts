// zaina-platform/src/connectors/tbm/tools.ts
//
// TBM's tools: the business logic the model calls for Tembea Bila Matata.
// Copied from the TBM app's server/zaina/tools.ts. TBM's listings, prices and
// bookings are read and written through TBM's own code (tbm-app.ts); the
// conversation itself (currency, what the customer typed, earlier tool
// results) comes from the platform's conversation store.
//
// DESIGN RULES:
...
import { phoneNumbersWritten, resolveAgentContact, resolveCustomerContact, sharesPhoneNumber, textArg } from "../../engine/tool-args.ts";
import {
  budgetBasisArg,
  ...
  timeArg,
  type RequestDetail,
} from "../../engine/custom-offer-intake.ts";
import {
  customerMessages as storedCustomerMessages,
  sessionCurrency,
  toolErrorResponses,
...
async function getSessionCurrency(sessionId: string): Promise<"USD" | "KES"> {
  return await sessionCurrency(sessionId);
}
...
export const bookingInventoryColumns = {
...
async function notifyBookingCreated(args: {
  ...
  try {
    transcript = await storedTranscript(args.sessionId);
  } catch (transcriptErr) {
 ...
 */
/**
 * The name, email and phone for a booking, only as the customer typed them.
 * The confirmation and payment link go to this email, so a detail the model
 * filled in on its own ("Guest", guest@example.com) is never used: the
 * customer is asked instead.
 */
async function bookingContact(
  args: { customer_name?: unknown; customer_email?: unknown; customer_phone?: unknown },
  sessionId: string,
): Promise<{ ok: true; name: string; email: string; phone: string } | { ok: false; response: Record<string, unknown> }> {
  ...
  if (!contact.ok) {
    const tellCustomer = contact.missing.length === 2
      ? "May I have your full name and email address for the booking? The confirmation and payment link go there."
      : contact.missing[0] === "email"
        ? "What email should I use for your booking confirmation?"
        : "May I have your full name for the booking, please?";
    return {
      ok: false,
      response: { ok: false, error: "customer_contact_required", missing: contact.missing, tell_customer: tellCustomer },
    };
  }
  if (!contact.phone) {
    return {
      ...
      response: {
        ...
        hint: "Ask the customer for their phone number; never fill one in.",
        tell_customer: "One more — what's the best phone number to reach you on?",
      },
    };
  }
  return { ok: true, name: contact.name, email: contact.email, phone: contact.phone };
}
...
export async function createDraftBooking(
  ...
  const contact = await bookingContact(args, sessionId);
  if (!contact.ok) return contact.response;
  args = { ...args, customer_name: contact.name, customer_email: contact.email, customer_phone: contact.phone };
...
export async function createServiceBooking(
  ...
  // 1. Contact details, only as the customer typed them
  const contact = await bookingContact(args, sessionId);
  if (!contact.ok) return contact.response;
  args = { ...args, customer_name: contact.name, customer_email: contact.email, customer_phone: contact.phone };
...
async function getCustomerMessages(sessionId: string): Promise<string[]> {
  return await storedCustomerMessages(sessionId);
}
...
async function hasAskedForAgentContact(sessionId: string): Promise<boolean> {
  return (await toolErrorResponses(sessionId, "create_listing_verification_request", "agent_contact_required")).length > 0;
}
...
async function customRequestDetailsAskedBefore(sessionId: string): Promise<RequestDetail[]> {
  const responses = await toolErrorResponses(sessionId, "create_custom_offer", "request_details_missing");
  return responses.flatMap((response) => {
    const missing = (response as { missing_details?: unknown } | null)?.missing_details;

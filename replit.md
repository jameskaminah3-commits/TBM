...
- **API Structure**: Public routes (`/api/stays`, `/api/cars`, etc.) provide type-safe data fetching for specific service domains. Admin routes (`/api/admin/*`) are protected by `requireAdmin` middleware and offer domain-specific CRUD operations for listings, bookings, and blog posts.
- **Data Models**: Separate domain tables (`Stays`, `Cars`, `Cooks`, `Errands`) with service-specific fields ensure accurate data representation. Shared models include `Users` (with role), `Sessions`, `Bookings`, and `BlogPosts`.
...
- **Data Integrity**: Real-time cache invalidation ensures admin CRUD changes immediately reflect on the frontend.

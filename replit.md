...
- **API Structure**: Public routes (`/api/stays`, `/api/cars`, etc.) provide type-safe data fetching for specific service domains. Admin routes (`/api/admin/*`) are protected by `requireAdmin` middleware and offer domain-specific CRUD operations for listings, bookings, and blog posts. Individual item fetch endpoints (`GET /api/admin/{entity}/:id`) enable edit form data loading with 404/500 error handling.

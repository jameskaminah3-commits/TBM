...
export default function ProviderExperienceNew() {
  ...
  const mutation = useMutation({
    mutationFn: async (data: FormData) => apiRequest("POST", "/api/provider/experiences", data),
    onSuccess: () => {
      ...
      toast({ title: "Experience submitted", description: "Your listing is private until admin reviews it." });
  ...
  return (
    <ProviderLayout>
      <div className="mx-auto max-w-5xl p-8">
        <div className="mb-8">
          <h1 className="mb-2 text-3xl font-serif font-semibold">Add Experience Listing</h1>
          <p className="text-muted-foreground">Build a flexible private offer, a shared group trip, or both for admin review.</p>
        </div>
        <Card>
          <CardHeader>
            <CardTitle>Experience Details</CardTitle>
            <CardDescription>Describe the story, guest fit, and departure structure clearly.</CardDescription>
          </CardHeader>
          <CardContent>
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                ...
                <div className="grid gap-4 md:grid-cols-3">
                  <FormField control={form.control} name="location" render={({ field }) => (
                    <FormItem><FormLabel>Provider Base Location</FormLabel><FormControl><Input placeholder="Mombasa, Watamu, Malindi..." {...field} /></FormControl><FormDescription>Where you are based as the host.</FormDescription><FormMessage /></FormItem>
                  )} />
                  <FormField control={form.control} name="experienceLocation" render={({ field }) => (
                    <FormItem><FormLabel>Experience Destination</FormLabel><FormControl><Input placeholder="Mambrui dunes, Old Town, marine park..." {...field} /></FormControl><FormDescription>Where the guest will actually go for the experience.</FormDescription><FormMessage /></FormItem>
                  )} />
                  <FormField control={form.control} name="experienceType" render={({ field }) => (
                    <FormItem><FormLabel>Experience Type</FormLabel><FormControl><Input placeholder="Sunset drive, marine outing, cultural trail" {...field} /></FormControl><FormMessage /></FormItem>
                  )} />
                </div>
                <div className="grid gap-4 md:grid-cols-3">
                ...
                <FormField control={form.control} name="meetingPoint" render={({ field }) => (
                  <FormItem><FormLabel>Meeting Point</FormLabel><FormControl><Input placeholder="Hotel lobby, marina entrance..." {...field} value={field.value ?? ""} /></FormControl><FormMessage /></FormItem>
                )} />
                ...
                {privateEnabled ? (
                  <div className="space-y-4 rounded-xl border p-5">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">For couples, families, and friends who want privacy and flexible timing.</p>
                    </div>
                    <div className="grid gap-4 md:grid-cols-2">
                      ...
                      <FormItem>
                        <FormControl>
                          <ExperienceAddonEditor
                            ...
                            description="Optional upgrades for the private experience."
                ...
                {sharedEnabled ? (
                  <div className="space-y-4 rounded-xl border p-5">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">Lower per-person pricing, fixed departures, and a small capped group.</p>
                    </div>
                    <div className="grid gap-4 md:grid-cols-3">
                      ...
                      <FormItem>
                        <FormControl>
                          <ExperienceAddonEditor
                            ...
                            description="Extras guests can attach when joining a shared departure."
                            value={Array.from((field.value ?? []) as ArrayLike<{ id: string; name: string; price: number }>)}
                            onChange={field.onChange}
                          />
                        </FormControl>
                ...
                <FormField control={form.control} name="customQuoteEnabled" render={({ field }) => (
                  <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4">
                    <div><FormLabel>Allow custom quote requests</FormLabel><FormDescription>Useful when guests may want a tailored version.</FormDescription></div>
                    <FormControl><Checkbox checked={field.value} onCheckedChange={field.onChange} /></FormControl>
                  </FormItem>
                )} />
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  ...
                  <FormField control={form.control} name="exclusionsText" render={({ field }) => (
                    <FormItem><FormLabel>Exclusions</FormLabel><FormControl><Textarea rows={5} placeholder={"Hotel pickup\nPersonal purchases"} {...field} value={field.value ?? ""} /></FormControl><FormDescription>One item per line.</FormDescription><FormMessage /></FormItem>
                  )} />
                </div>
